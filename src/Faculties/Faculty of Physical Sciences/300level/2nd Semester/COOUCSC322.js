...
import { collectionId, databaseId, databases2, Query, bucketId, projectId2 } from '../../../../../screens/UploadScreens/config';

const COOUCSC322 = ({ title = "COOUCSC322 - Introduction to Computer Science"}) => {
      ...
      try {
        ...
        const response = await databases2.listDocuments(
          ...
          [
            ...
            Query.equal('course', 'COOUCSC322'),
          ]
        );

        if (response.documents.length > 0) {
          const fileId = response.documents[0].fileId;
          const downloadUrl = `https://cloud.appwrite.io/v1/storage/buckets/${bucketId}/files/${fileId}/download?project=${projectId2}`;
          setPdfUrl(downloadUrl);
        } else {
          setError('No PDF found for COOUCSC322 course');
        }
      } catch (err) {
