...
import { collectionId, databaseId, databases3, Query, bucketId, projectId3 } from '../../../../../screens/UploadScreens/config';

const CSC404 = ({ title = "CSC404 - Introduction to Computer Science"}) => {
      ...
      try {
        ...
        const response = await databases3.listDocuments(
          ...
          [
            ...
            Query.equal('course', 'CSC404'),
          ]
        );

        if (response.documents.length > 0) {
          const fileId = response.documents[0].fileId;
          const downloadUrl = `https://cloud.appwrite.io/v1/storage/buckets/${bucketId}/files/${fileId}/download?project=${projectId3}`;
          setPdfUrl(downloadUrl);
        } else {
          setError('No PDF found for CSC404 course');
        }
      } catch (err) {
