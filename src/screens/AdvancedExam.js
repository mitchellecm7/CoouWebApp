...
import '../screens/UploadScreens/styles/AdvancedExam.css';
...
const AdvancedExam = () => {
  ...
  const getInitialQuestions = async () => {
    ...
    const mockQuestions = Array.from({ length: QUESTION_SIZE }, (_, i) => ({
      ...
      option5: `Option E for Q${i + 1}`,
      correctOptions: [1, 2] // Example multiple correct options
    }));
  ...
  const handleOptionSelect = (questionIndex, option) => {
    ...
    if (currentSelections.includes(option)) {
      setSelectedOptions({
        ...selectedOptions,
        [questionIndex]: currentSelections.filter((opt) => opt !== option),
      });
    } else {
      setSelectedOptions({
        ...selectedOptions,
        [questionIndex]: [...currentSelections, option],
      });
    }
  };
  ...
  const handleSubmit = () => {
    ...
    questions.forEach((question, index) => {
      const userSelectedOptions = selectedOptions[index] || [];
  ...
  return (
    <div className="advanced-exam-container">
              ...
              <div className="options-container">
                {[1, 2, 3, 4, 5].map((optionNum) => (
                  <button
                    key={optionNum}
                    className={`option ${
                      selectedOptions[index]?.includes(optionNum) ? 'selected' : ''
                    } ${
                      showResults && item.correctOptions.includes(optionNum) ? 'correct' : ''
                    } ${
                      showResults && selectedOptions[index]?.includes(optionNum) &&
                      !item.correctOptions.includes(optionNum) ? 'wrong' : ''
                    }`}
